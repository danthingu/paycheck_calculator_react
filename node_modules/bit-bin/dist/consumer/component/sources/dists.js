 ...
 */
class Dists {
  // changed only when importing a component
  // populated only after getDistDirForConsumer() is called
  constructor(dists, mainDistFile) {
    (0, _defineProperty2().default)(this, "dists", void 0);
    (0, _defineProperty2().default)(this, "writeDistsFiles", true);
    (0, _defineProperty2().default)(this, "areDistsInsideComponentDir", true);
    (0, _defineProperty2().default)(this, "distEntryShouldBeStripped", false);
    (0, _defineProperty2().default)(this, "_mainDistFile", void 0);
    (0, _defineProperty2().default)(this, "distsRootDir", void 0);
  ...
  getDistDirForConsumer(consumer, componentRootDir) {
    if (consumer.shouldDistsBeInsideTheComponent()) {
      this.distsRootDir = path().join(componentRootDir, _constants().DEFAULT_DIST_DIRNAME);
    } else {
  ...
  hasFileParallelToSrcFile(srcFile) {
    const distFile = (0, _utils().searchFilesIgnoreExt)(this.dists, path().normalize(srcFile));
    return Boolean(distFile);
  }

  static getDistDirWhenDistIsOutsideCompDir(workspaceConfig, componentRootDir) {
    if (workspaceConfig._distEntry) componentRootDir = componentRootDir.replace(workspaceConfig._distEntry, '');

    const distTarget = workspaceConfig._distTarget || _constants().DEFAULT_DIST_DIRNAME;

    return path().join(distTarget, componentRootDir);
  }

  getDistDir(consumer, componentRootDir) {
    if (consumer) return this.getDistDirForConsumer(consumer, componentRootDir);
    this.distsRootDir = path().join(componentRootDir, _constants().DEFAULT_DIST_DIRNAME);
    return this.distsRootDir;
  }

  updateDistsPerWorkspaceConfig(id, consumer, componentMap) {
    if (this.isEmpty()) return;
    const newDistBase = this.getDistDir(consumer, componentMap.getRootDir());
    this.dists.forEach(dist => dist.updatePaths({
      newBase: newDistBase
    }));
    if (consumer) this.stripDistEntryIfNeeded(id, consumer, componentMap);
  }

  stripDistEntryIfNeeded(id, consumer, componentMap) {
    ...
    const shouldDistEntryBeStripped = () => {
      ...
      if (!distEntry || componentMap.origin === _constants().COMPONENT_ORIGINS.NESTED) return false;
    ...
    _logger().default.debug(`stripping dist.entry "${distEntry}" from ${id.toString()}`);

    this.distEntryShouldBeStripped = true; // @ts-ignore AUTO-ADDED-AFTER-MIGRATION-PLEASE-FIX!

    this.dists.forEach(dist => dist.updatePaths({
      newRelative: dist.relative.replace(distEntry, '')
    }));
  ...
  stripOriginallySharedDir(originallySharedDir) {
    this.dists.forEach(distFile => {
      // @ts-ignore AUTO-ADDED-AFTER-MIGRATION-PLEASE-FIX!
      const newRelative = (0, _manipulateDir().stripSharedDirFromPath)(distFile.relative, originallySharedDir);
      distFile.updatePaths({
        newRelative
      });
    });
    this._mainDistFile = this._mainDistFile ? (0, _manipulateDir().stripSharedDirFromPath)(this._mainDistFile, originallySharedDir) : this._mainDistFile;
  }
  /**
  ...
  writeDists(component, consumer, // @ts-ignore AUTO-ADDED-AFTER-MIGRATION-PLEASE-FIX!
  writeLinks = true) {
    var _this = this;

    return (0, _bluebird().coroutine)(function* () {
      const dataToPersist = yield _this.getDistsToWrite(component, consumer.bitMap, consumer, writeLinks);
      if (!dataToPersist) return null;
      if (consumer) dataToPersist.addBasePath(consumer.getPath());
      yield dataToPersist.persistAllToFS(); // @ts-ignore AUTO-ADDED-AFTER-MIGRATION-PLEASE-FIX!

      return _this.dists.map(distFile => distFile.path);
    })();
  }
  /**
  ...
  writeDistsLinks(component, consumer) {
    var _this2 = this;

    return (0, _bluebird().coroutine)(function* () {
      if (_this2.isEmpty() || !_this2.writeDistsFiles) return;
      const componentMap = consumer ? consumer.bitMap.getComponent(component.id, {
        ignoreVersion: true
      }) : component.componentMap;
      if (!componentMap) throw new Error('writeDistsLinks expect componentMap to be defined');
      if (componentMap.origin === _constants().COMPONENT_ORIGINS.NESTED) return;
      const dataToPersist = yield (0, _links().getLinksInDistToWrite)(component, componentMap, consumer, consumer.bitMap);
  ...
  getDistsToWrite(component, bitMap, consumer, // @ts-ignore AUTO-ADDED-AFTER-MIGRATION-PLEASE-FIX!
  writeLinks = true, componentWithDependencies) {
    var _this3 = this;

    return (0, _bluebird().coroutine)(function* () {
      if (_this3.isEmpty() || !_this3.writeDistsFiles) return null;
      ...
      _this3.updateDistsPerWorkspaceConfig(component.id, consumer, componentMap);

      dataToPersist.addManyFiles(_this3.dists);

      if (writeLinks && componentMap && componentMap.origin === _constants().COMPONENT_ORIGINS.IMPORTED) {
        const linksInDist = yield (0, _links().getLinksInDistToWrite)(component, componentMap, consumer, bitMap, componentWithDependencies);
  ...
  calculateMainDistFile(mainSourceFile) {
    if (this.writeDistsFiles && this.areDistsInsideComponentDir) {
      const getMainFile = () => {
        if (this._mainDistFile) return this._mainDistFile; // Take the only dist file if there is only one or search for one with the same name as the main source file

        if (this.dists && this.dists.length === 1) return this.dists[0].relative; // @ts-ignore AUTO-ADDED-AFTER-MIGRATION-PLEASE-FIX!

        return (0, _utils().searchFilesIgnoreExt)(this.dists, mainSourceFile, 'relative');
      };

      const mainFile = getMainFile();
      if (mainFile) return path().join(_constants().DEFAULT_DIST_DIRNAME, mainFile);
    }
  ...
  calculateDistFileForAuthored(componentFile, consumer, isMain) {
    if (this.isEmpty()) return componentFile;

    const getFileToSearch = () => {
    ...
    const fileToSearch = getFileToSearch(); // @ts-ignore AUTO-ADDED-AFTER-MIGRATION-PLEASE-FIX!

    const distFile = isMain && this._mainDistFile ? this._mainDistFile : (0, _utils().searchFilesIgnoreExt)(this.dists, fileToSearch, 'relative');
    if (!distFile) return componentFile;

    const distTarget = consumer.config._distTarget || _constants().DEFAULT_DIST_DIRNAME;

    return path().join(distTarget, distFile);
  }

  toDistFilesModel(consumer, originallySharedDir, isCompileSet) {
    ...
    const addSharedDirAndDistEntry = pathStr => {
      ...
      const withDistEntry = this.distEntryShouldBeStripped ? path().join(consumer.config._distEntry, withSharedDir) : withSharedDir;
      return (0, _path2().pathNormalizeToLinux)(withDistEntry);
    };
    ...
    const dists = this.get().map(dist => {
      return {
        ...
        // @ts-ignore AUTO-ADDED-AFTER-MIGRATION-PLEASE-FIX!
        file: _source().default.from(dist.contents),
        // @ts-ignore AUTO-ADDED-AFTER-MIGRATION-PLEASE-FIX!
        test: dist.test
      };
    });
    const mainDistFile = this._mainDistFile ? addSharedDirAndDistEntry(this._mainDistFile) : this._mainDistFile; // @ts-ignore AUTO-ADDED-AFTER-MIGRATION-PLEASE-FIX!
  ...
  static getNodePathDir(consumer) {
    const resolveModules = consumer.config._resolveModules;
    if (!resolveModules || !resolveModules.modulesDirectories || !resolveModules.modulesDirectories.length) return undefined;

    const distTarget = consumer.config._distTarget || _constants().DEFAULT_DIST_DIRNAME;

    const distEntry = consumer.config._distEntry;
    const nodePaths = resolveModules.modulesDirectories.map(moduleDir => {
      const isRelative = str => str.startsWith('./') || str.startsWith('../');

      if (!distEntry) return path().join(distTarget, moduleDir);
      const distEntryRelativeToModuleDir = (0, _utils().pathRelativeLinux)(distEntry, moduleDir);
    ...
    return nodePaths.map(nodePath => consumer.toAbsolutePath(nodePath)).map(_path2().pathNormalizeToLinux).join(_constants().NODE_PATH_SEPARATOR);
  }
