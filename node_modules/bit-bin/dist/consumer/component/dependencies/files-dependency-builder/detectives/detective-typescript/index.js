...
function _parserHelper() {
  const data = require("../parser-helper");

  _parserHelper = function () {
    return data;
  };
...
function _default(src, options = {}) {
  ...
  const addAngularLocalDependency = dependency => {
    const angularDep = (0, _utils().isRelativeImport)(dependency) ? dependency : `./${dependency}`;
  ...
  walker.walk(src, function (node) {
    switch (node.type) {
      case 'ImportDeclaration':
        if (node.source && node.source.value) {
          ...
          node.specifiers.forEach(specifier => {
            const specifierValue = (0, _parserHelper().getSpecifierValueForImportDeclaration)(specifier);
      ...
      case 'CallExpression':
        {
          const value = (0, _parserHelper().getDependenciesFromCallExpression)(node);
      ...
      case 'MemberExpression':
        {
          const value = (0, _parserHelper().getDependenciesFromMemberExpression)(node);
          if (value) addDependency(value);
        }
        break;

      case 'Decorator':
        // parse Angular Decorators to find style/template dependencies
        if (node.expression && node.expression.callee && node.expression.callee.name === 'Component' && node.expression.arguments && node.expression.arguments.length && node.expression.arguments[0].type === 'ObjectExpression') {
