...
function _getConsumerInfo() {
  _getConsumerInfo = (0, _bluebird().coroutine)(function* (absPath) {
    ...
    for (let i = 0; i < searchPaths.length; i += 1) {
      ...
      const hasConsumerConfig = yield pathHasConsumerConfig(path); // eslint-disable-line no-await-in-loop
    ...
    function pathHasConsumerConfig(_x4) {
      return _pathHasConsumerConfig.apply(this, arguments);
    }

    function _pathHasConsumerConfig() {
      _pathHasConsumerConfig = (0, _bluebird().coroutine)(function* (path) {
        const isExist = yield _config().LegacyWorkspaceConfig.isExist(path);
        return !!isExist;
      });
      return _pathHasConsumerConfig.apply(this, arguments);
    }
