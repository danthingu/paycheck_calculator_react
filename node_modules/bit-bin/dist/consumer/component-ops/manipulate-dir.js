...
exports.addSharedDirForPath = addSharedDirForPath;
...
function _getManipulateDirForExistingComponents() {
  _getManipulateDirForExistingComponents = (0, _bluebird().coroutine)(function* (consumer, componentVersion) {
    ...
    const wrapDir = componentMap ? getWrapDirIfNeeded(componentMap.origin, version) : undefined; // @ts-ignore AUTO-ADDED-AFTER-MIGRATION-PLEASE-FIX!
...
function _getManipulateDirWhenImportingComponents() {
  _getManipulateDirWhenImportingComponents = (0, _bluebird().coroutine)(function* (bitMap, versionsDependencies, repository) {
    ...
    const manipulateDirDataP = versionsDependencies.map( /*#__PURE__*/function () {
...
function getManipulateDirForConsumerComponent(component) {
  ...
  const wrapDir = isWrapperDirNeededForConsumerComponent(component) ? _constants().WRAPPER_DIR : undefined;
  return {
    id,
    originallySharedDir,
    wrapDir
...
function getOriginallySharedDirIfNeeded(origin, version) {
  if (origin === _constants().COMPONENT_ORIGINS.AUTHORED || version.ignoreSharedDir) return undefined;
...
function getWrapDirIfNeeded(origin, version) {
  if (origin === _constants().COMPONENT_ORIGINS.AUTHORED) return undefined;
  return isWrapperDirNeeded(version) ? _constants().WRAPPER_DIR : undefined;
}
/**
