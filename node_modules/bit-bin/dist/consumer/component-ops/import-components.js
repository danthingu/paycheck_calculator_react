...
class ImportComponents {
  ...
  _filterComponentsWithLowerVersions(componentsWithDependencies) {
    return componentsWithDependencies.filter(comp => {
      const sameIdHigherVersion = componentsWithDependencies.find(c => !c.component.id.isEqual(comp.component.id) && c.component.id.isEqualWithoutVersion(comp.component.id) && // eslint-disable-next-line @typescript-eslint/no-non-null-assertion
      _semver().default.gt(c.component.id.version, comp.component.id.version));
  ...
  _getBitIds() {
    ...
    return (0, _bluebird().coroutine)(function* () {
      const bitIds = [];
      yield Promise.all(_this2.options.ids.map( /*#__PURE__*/function () {
  ...
  _getCurrentVersions(ids) {
    ...
    return (0, _bluebird().coroutine)(function* () {
      const versionsP = ids.map( /*#__PURE__*/function () {
  ...
  _getImportDetails(currentVersions, components) {
    ...
    return (0, _bluebird().coroutine)(function* () {
      const detailsP = components.map( /*#__PURE__*/function () {
  ...
  _throwForModifiedOrNewComponents(ids) {
    ...
    return (0, _bluebird().coroutine)(function* () {
      ...
      if (_this8.options.override || _this8.options.objectsOnly || _this8.options.merge) return;
      const componentsStatuses = yield _this8.consumer.getManyComponentsStatuses(ids);
