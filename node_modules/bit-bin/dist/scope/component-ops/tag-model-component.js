...
function updateDependenciesVersions(componentsToTag) {
  const updateDependencyVersion = (dependency, idFieldName = 'id') => {
    const foundDependency = componentsToTag.find(component => component.id.isEqualWithoutVersion(dependency[idFieldName]));
  ...
  componentsToTag.forEach(oneComponentToTag => {
    oneComponentToTag.getAllDependencies().forEach(dependency => updateDependencyVersion(dependency)); // TODO: in case there are core extensions they should be excluded here
    ...
    oneComponentToTag.extensions.forEach(extension => {
      ...
      if (extension.extensionId) {
        ...
        if (!wasDependencyFound && !extension.extensionId.hasScope() && !extension.extensionId.hasVersion()) {
          throw new (_generalError().default)(`fatal: "${oneComponentToTag.id.toString()}" has an extension "${extension.extensionId.toString()}".
...
function _setFutureVersions() {
  _setFutureVersions = (0, _bluebird().coroutine)(function* (componentsToTag, scope, releaseType, exactVersion) {
    yield Promise.all(componentsToTag.map( /*#__PURE__*/function () {
...
var _default = /*#__PURE__*/function () {
  var _tagModelComponent = (0, _bluebird().coroutine)(function* ({
    ...
    if (!ignoreNewestVersion) {
      const newestVersionsP = componentsToBuildAndTest.map( /*#__PURE__*/function () {
    ...
    if (nonLegacyComps.length) {
      ...
      yield Promise.all(scope.onTag.map(func => func(ids)));
    }
    ...
    const persistComponent = /*#__PURE__*/function () {
