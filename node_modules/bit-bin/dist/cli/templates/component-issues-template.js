...
const componentIssuesLabels = {
  missingPackagesDependenciesOnFs: 'missing packages dependencies (use `bit install` to make sure all package dependencies are installed)',
  ...
  relativeComponentsAuthored: 'components with relative import statements found (replace to module paths or use "bit link --rewire" to replace)',
...
function formatMissing(missingComponent) {
  ...
  const missingStr = Object.keys(componentIssuesLabels).map(key => {
    ...
    if (key === 'untrackedDependencies') {
      // @ts-ignore
      return formatMissingStr(key, // @ts-ignore
      missingComponent.issues[key], componentIssuesLabels[key], untrackedFilesComponentIssueToString);
    }

    if (key === 'relativeComponentsAuthored') {
      return formatMissingStr(key, // @ts-ignore
      missingComponent.issues[key], componentIssuesLabels[key], relativeComponentsAuthoredIssuesToString);
    }
