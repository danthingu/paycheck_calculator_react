...
var Bundler = /** @class */ (function () {
    ...
    Bundler.prototype.cleanUnused = function (meta, usedPaths) {
        var _this = this;
        var transformer = this.stylable.createTransformer();
        meta.ast.walkRules(function (rule) {
            var outputSelectors = rule.selectors.filter(function (selector) {
                return _this.isInUse(transformer, meta, selector, usedPaths);
            });
    ...
    Bundler.prototype.isInUse = function (transformer, meta, selector, usedPaths) {
        var _this = this;
        var selectorElements = transformer.resolveSelectorElements(meta, selector);
        // We expect to receive only one selectors at a time
        return selectorElements[0].every(function (res) {
            var lastChunk = res.resolved[res.resolved.length - 1];
            if (lastChunk) {
                var source = _this.stylable.resolvePath(undefined, lastChunk.meta.source);
